...
//debug: change filename from "/proc/self/cgroup" -> "/tmp/proc-self-cgroup.txt"
	if err != nil {
		//debug:fmt.Println("error opening file: /proc/self/cgroup")
		return ""
	}
	fmt.Println("file opened: /proc/self/cgroup")
	...
	for scanner.Scan() {
		...
		if err == nil {
			...
			if id := matchDockerCurrentContainerID(strLines); id != "" {
				//debug:fmt.Println("id: ",id)
				return id //below was missed (after else if) " id :="
			} else if id := matchECSCurrentContainerID(strLines); id != "" {
				//debug:fmt.Println("ECS id: ",id)
				return id
			}
		}
	}
...
func matchDockerCurrentContainerID(lines string) string {
	...
	if re.MatchString(lines) {
		...
		//debug:fmt.Println("containerID: ",containerID)
		return containerID
	}
	return ""
}
...
func matchECSCurrentContainerID(lines string) string {
	...
	if re.MatchString(string(lines)) {
		submatches := re.FindStringSubmatch(string(lines))
		containerID := submatches[1]
		//debug:fmt.Println("ECS containerID: ",containerID)
		return containerID
	}
